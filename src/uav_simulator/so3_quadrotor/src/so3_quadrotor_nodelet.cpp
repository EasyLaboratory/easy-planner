...
class Nodelet : public nodelet::Nodelet {
    ...
    if (tnow >= next_odom_pub_time) {
      ...
      for (size_t i = 0; i < 4; ++i) {
        ...
        Eigen::Quaterniond quat_propeller =
            quat *
            uav_utils::ypr_to_quaternion(Eigen::Vector3d(motor_yaw_[i], 0, 0));
        Eigen::Vector3d pos_propeller =
            pos + quat.toRotationMatrix() * propellers[i];
 ...
 public:
  void onInit(void) {
    ...
    cmd_sub_ = nh.subscribe<quadrotor_msgs::SO3Command>(
        "so3cmd", 10, &Nodelet::cmd_callback, this,
        ros::TransportHints().tcpNoDelay());
    simulation_timer = nh.createTimer(ros::Duration(1.0 / simulation_rate_),
                                      &Nodelet::timer_callback, this);
