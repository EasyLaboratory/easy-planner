  ...
  if (add_ground) {
    ...
    for (int i = 0; i < info.sizeX; ++i) {
      for (int j = 0; j < info.sizeY; ++j) {
        info.cloud->points.emplace_back(_x_l + i * 1.0 / info.scale,
                                        _y_l + j * 1.0 / info.scale, 0);
      }
    }
  }
}
    ...
    for (int j = 0; j < info.sizeY; ++j) {
      for (int k = 0; k < info.sizeZ; ++k) {
        ...
        for (int it = 1; it <= fractal; ++it) {
          ...
          tnoise += ta * noise.noise(dfv * i * complexity, dfv * j * complexity,
                                     dfv * k * complexity);
        }
        v->push_back(tnoise);
      }
...
void Maps::recursiveDivision(int xl, int xh, int yl, int yh,
                             Eigen::MatrixXi& maze) {
  ROS_INFO("generating maze with width %d , height %d", xh - xl + 1,
           yh - yl + 1);

  if (xl < xh - 3 && yl < yh - 3) {  // the remaining area is larger than or
                                     // equal to 5*5, need to add both x
    // wall and y wall
    ...
    while (valid == false) {
      ...
      if (xl - 1 >= 0) {  // there is a point at xl-1,ym
        if (maze(xl - 1, ym) ==
            0) {  // this is an opening,need to change random number
          continue;
        }
      }
    ...
    int d4 = std::rand() % (yh - ym) + ym +
             1;  // generating four possible door points
    ...
    ROS_INFO("finished generating maze with width %d , height %d", xh - xl + 1,
             yh - yl + 1);
    std::cout << maze << std::endl;
    return;
    ...
    ym = (std::rand() % (yh - yl - 1) + yl +
          1);  // generating random number between yl+1 and yh-1(pointless to
               // add a wall at the sides)
               // xm and ym are now the valid coordinate of the center of the
               // wall
    for (int i = xl; i <= xh; i++) {
      maze(i, ym) = 1;
    }
...
void Maps::Maze3DGen() {
  ...
  for (int i = 0; i < info.sizeX; i++) {
    for (int j = 0; j < info.sizeY; j++) {
      for (int k = 0; k < info.sizeZ;
           k++) {  // for every scaled coordinate points
        pcl::PointXYZ test;
        ...
        if (std::abs(mp.getDist2() - mp.getDist1()) <
            1 / info.scale) {  // the tested location is on one of the middle
                               // planes
