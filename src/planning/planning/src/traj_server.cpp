...
void publish_cmd(int traj_id, const Eigen::Vector3d &p,
  ...
  ROS_INFO("TO airsim point (x, y, z) = (%f, %f, %f), (vx, vy, vz) = (%f, %f, %f), yaw = %f)", point.position.x,
           point.position.y, point.position.z, point.velocity.x,
           point.velocity.y, point.velocity.z, point.yaw);
